document.addEventListener('DOMContentLoaded', function() {
  ...
  categoryButtons.forEach(btn => {
    ...
    itemsPerCategory[category] = document.querySelectorAll(`.coffee-item[data-category="${category}"]`).length;
  });
  ...
  function updateCategory(category) {
    ...
    document.querySelectorAll(`.coffee-item[data-category="${category}"]`).forEach(item => {
      item.classList.add('category-active');
    });
    
    // Update items array with ONLY current category items
    items = Array.from(document.querySelectorAll(`.coffee-item[data-category="${currentCategory}"]`));
  ...
  function updateDots() {
    ...
    // Always show exactly 3 dots regardless of item count
    for (let i = 0; i < dots.length; i++) {
      if (i < 3) {
        dots[i].style.display = 'inline-block';
        // Calculate which dot should be active based on modulo position
        const dotPosition = Math.floor(currentSlideIndex % 3);
  ...
  dots.forEach((dot, index) => {
    dot.addEventListener('click', () => {
      if (items.length > 0) {
        ...
        const currentPage = Math.floor(currentSlideIndex / 3);
        currentSlideIndex = (currentPage * 3 + index) % items.length;
        updateCarousel();
        updateDots();
      }
    });
